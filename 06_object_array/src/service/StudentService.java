...
public class StudentService {
	...
	public StudentService() {
		...
		for(StudentDTO std : students) {
			...
			// random.nextInt(101) : 0 이상 101미만의 정수형 난수 생성
	 ...
	 * (students 에 저장된 주소가 복사되서 반환됨 == 얕은 복사)
	 * @return students
	 */
	...
	public boolean addStudent(StudentDTO student) {
		...
		// students 객체 배열에서 null 인 인덱스 찾기
	...
	/** targetName과 이름이 일치하는 학생 객체 봔환 service 메서드
	 * @param targetName 
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			...
			//  + std.getName()시 String 반환 기본자료형에 없으므로 참조형 = 주소를 반환한다
			//  + 객체는 생성시 서로 다른 곳에 생성됨
	...
	/** 전달 받은 index가 studetns 범위 내 인덱스가 맞는지
	...
	public String selectMaxMin() {
		...
		for(StudentDTO std : students) { // 향상된 for 무
